...
class Plugin
{
    ...
    public static function handle_amelia_employee_statistics(): void
    {
        // 检查安全性
        if (!current_user_can('amelia_read_appointments')) {
        ...
        if (!self::validate_amelia_date_range($start_date, $end_date)) {
        ...
        AND type = 'provider'",
            ...
            $employee_id = $wpdb->get_var($employee_query);
        ...
        AND a.status = 'approved'",
            ...
            $totals = $wpdb->get_row($appointments_query);
            ...
            SUM(cb.price) as service_revenue
        FROM {$wpdb->prefix}amelia_services s
        INNER JOIN {$wpdb->prefix}amelia_appointments a ON s.id = a.serviceId
        INNER JOIN {$wpdb->prefix}amelia_customer_bookings cb ON a.id = cb.appointmentId
        WHERE a.providerId = %d 
        AND DATE(a.bookingStart) BETWEEN %s AND %s
        AND a.status = 'approved'
        GROUP BY s.id",
            ...
            $services_stats = $wpdb->get_results($services_query);

            // 3. 计算平均收入
            $average_income = 0;
            if ($totals->total_appointments > 0) {
                $average_income = $totals->total_revenue / $totals->total_appointments;
            }
            ...
            foreach ($services_stats as $service) {
                $formatted_services[] = array(
                    ...
                    'appointments' => (int)$service->appointment_count,
            ...
            $response = array(
                'success' => true,
                'data' => array(
                    'totalAppointments' => (int)$totals->total_appointments,
                    'totalRevenue' => (float)$totals->total_revenue,
                    'averageIncome' => (float)$average_income,
            ...
            wp_send_json_error('Error processing statistics: ' . $e->getMessage());
        }

    }

    public static  function validate_amelia_date_range($start_date, $end_date): bool
    {
        // 验证日期格式
...
// 注册 Ajax 动作
add_action('wp_ajax_amelia_employee_statistics', ['AmeliaBooking\Plugin' ,'handle_amelia_employee_statistics']);
add_action('wp_ajax_nopriv_amelia_employee_statistics', ['AmeliaBooking\Plugin' ,'handle_amelia_employee_statistics']);
